...
import type { Feature, Domain, FeatureStatus } from "@/lib/matrix";
import type { ContractOverviewItem, ActivityItem } from "@/app/features/[id]/page";
...
const KIND_LABEL: Record<string, string> = {
  require: "需求澄清", code: "代码实现", test: "测试", deploy: "上线",
  analyze: "状态分析", "contract-sync": "契约同步", verify: "验证",
};
...
export function FeaturePageClient({ data }: { data: FeaturePageData }) {
  ...
  const hasImplementation = !!(feature.implementation.decider || feature.implementation.acceptor || feature.implementation.handler || feature.implementation.actions.length > 0 || feature.implementation.context);
  ...
  async function handleDeleteScenario(scenarioName: string) {
    ...
    const res = await fetch(`/api/features/${feature.id}/scenarios/${encodeURIComponent(scenarioName)}`, { method: "DELETE" });
    if (!res.ok) {
      const d = await res.json();
      alert(d.error ?? "删除失败");
      return;
    ...
    <div className="space-y-6">
      ...
      <header className="space-y-3 border-b border-border pb-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <span className={`inline-flex items-center rounded-md px-2 py-0.5 text-xs font-semibold ${priorityClasses[feature.priority] ?? priorityClasses.P2}`}>
            {feature.priority}
          </span>
        </div>
      </header>
          ...
          <div className="flex flex-wrap items-start gap-x-5 gap-y-2 text-xs">
            {feature.implementation.decider && (
              <div className="flex items-center gap-2">
                <span className="rounded bg-green-100 px-1.5 py-0.5 font-mono font-semibold text-green-700">Decider</span>
                <span className="font-mono text-foreground">{feature.implementation.decider}</span>
              </div>
            )}
            {feature.implementation.acceptor && (
              <div className="flex items-center gap-2">
                <span className="rounded bg-teal-100 px-1.5 py-0.5 font-mono font-semibold text-teal-700">Acceptor</span>
                <span className="font-mono text-foreground">{feature.implementation.acceptor}</span>
              </div>
            )}
            {feature.implementation.handler && (
              <div className="flex items-center gap-2">
                <span className="rounded bg-blue-100 px-1.5 py-0.5 font-mono font-semibold text-blue-700">Handler</span>
            ...
            {feature.implementation.actions.length > 0 && (
              <div className="flex items-start gap-2">
                <span className="rounded bg-purple-100 px-1.5 py-0.5 font-mono font-semibold text-purple-700">Actions</span>
                <span className="font-mono text-foreground">{feature.implementation.actions.join(", ")}</span>
              </div>
            )}
          </div>
        ) : (
          <div className="text-xs text-muted-foreground">待分析 — 需求澄清或扫码分析后将产出分层归属</div>
        )}
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-3">
          ...
          <button type="button" className="text-sm text-blue-600 hover:underline cursor-pointer" onClick={() => setAddScenarioOpen(true)}>
            + 添加场景
          </button>
        </div>
                ...
                return (
                  <tr key={s.name} className="border-b border-border">
                    <td className="px-3 py-2">
                      <Link href={`/features/${feature.id}/${encodeURIComponent(s.name)}`} className="font-medium text-blue-600 hover:underline">
                        {s.name}
                      </Link>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <div className="flex flex-col gap-0.5">
                        ...
                        {s.agentStatus !== "not-started" && (
                          <span className="text-[10px] text-muted-foreground">{AGENT_STATUS_LABEL[s.agentStatus] ?? s.agentStatus}</span>
                        )}
                      </div>
                    </td>
                    {/* 契约列 */}
                    ...
                    <td className="px-3 py-2 text-center">
                      <button type="button" className="text-xs text-red-600 hover:underline cursor-pointer" onClick={() => handleDeleteScenario(s.name)}>
                        删除
                      </button>
                    </td>
                  </tr>
              ...
              <tbody>
                {recentActivities.map((a) => (
                  <tr key={a.runId} className="border-b border-border hover:bg-muted/30">
                    <td className="px-3 py-2">
                      <span className={`inline-flex items-center rounded px-1.5 py-0.5 font-semibold ${ACTIVITY_STATE_BADGE[a.state] ?? "bg-gray-100 text-gray-600"}`}>
                        {a.state === "running" && <span className="mr-1 inline-block h-1.5 w-1.5 animate-pulse rounded-full bg-blue-500" />}
                        {a.state}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <Link href={`/runs/${a.runId}`} className="font-mono text-blue-600 hover:underline">{a.runId.slice(0, 16)}…</Link>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <AddScenarioDialog open={addScenarioOpen} onOpenChange={setAddScenarioOpen} featureId={feature.id} />
    </div>
  );
}
